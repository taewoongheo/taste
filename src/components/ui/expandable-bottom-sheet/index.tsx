...
// @ts-ignore
import type { ExpandableBottomSheetMethods, ExpandableBottomSheetProps } from './types';
...
const ExpandableBottomSheetComponent = forwardRef<
...
>(
  (
    ...
    // Internal snap points: user-defined + expanded
    const internalSnapPoints = useMemo<number[]>(
      () => [...parsedSnapPoints, expandedHeight],
      [parsedSnapPoints, expandedHeight],
    );

    const maxSnapPoint = useMemo<number>(
      () => Math.max(...internalSnapPoints),
      [internalSnapPoints],
    );

    const minSnapPoint = useMemo<number>(
      () => Math.min(...internalSnapPoints),
      [internalSnapPoints],
    );
    ...
    const userMaxSnapIndex = useMemo<number>(() => parsedSnapPoints.length - 1, [parsedSnapPoints]);
    ...
    const findClosestSnapPoint = useCallback(
      ...
      [internalSnapPoints, snapVelocityThreshold],
    );
    ...
    const snapToPoint = useCallback(
      (index: number, animated = true) => {
        ...
        // Enable scroll at user max snap or expanded
    ...
    const userMaxY = useMemo(
      () => SCREEN_HEIGHT - Math.max(...parsedSnapPoints),
      [parsedSnapPoints],
    );
...
export const ExpandableBottomSheet = memo<
  React.ForwardRefExoticComponent<
    ExpandableBottomSheetProps & React.RefAttributes<ExpandableBottomSheetMethods>
  >
